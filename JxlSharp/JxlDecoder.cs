...
namespace JxlSharp
{
	...
	public class JxlDecoder : IDisposable
	{
		...
		//commented out because you can just get the byte array without making a call to get its size first
		///// <summary>
		...
		{
			UnsafeNativeJxl.JxlColorEncoding colorEncoding2 = new UnsafeNativeJxl.JxlColorEncoding();
			UnsafeNativeJxl.CopyFields.ReadFromPublic(ref colorEncoding2, colorEncoding);
			unsafe
			{
		...
		{
			UnsafeNativeJxl.JxlPixelFormat pixelFormat2 = new UnsafeNativeJxl.JxlPixelFormat();
			UnsafeNativeJxl.CopyFields.ReadFromPublic(ref pixelFormat2, format);
			unsafe
			{
		...
		//Commented out because using a C# function as a callback is slow and complicated
		///// <summary>
